...
class TestService {
    ...
    void test_searchTable() throws Exception {
        ...
        when(repository.findTableColumnNames(tableName1)).thenReturn(columnNames1);
        when(repository.findTableColumnNames(tableName2)).thenReturn(columnNames2);
        when(repository.findTableColumnNames(tableName3)).thenReturn(columnNames3);

 ...
 * <p>The class is not meant to be instantiated. Instead, use its static methods to create repositories.</p>
 * <p>The class has a private constructor to prevent instantiation, and a static method `createRepository` that creates a repository based on the provided driver name.
 * The driver name is passed as a string, and the method uses a switch statement to create the appropriate type of repository.</p>
 * <p>Currently, the `createRepository` method supports two types of repositories: "postgresql" and "mysql". If an unsupported driver is provided, the method returns null.</p>
 * <p>This class can be used to create a repository without knowing the exact class of the repository that will be created. This can be useful in situations where the specific type of repository is not known until runtime.</p>
 * <p>Example usage:</p>
 * <p>RepositoryPort repository = RepositoryFactory.createRepository(connection, "postgresql");</p>
 */
